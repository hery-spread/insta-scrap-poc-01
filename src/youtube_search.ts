    ...
    for (const contentItem of contentItems) {
      ...
      if (
        ...
        if (watchMatch) {
          ...
          console.log(
            `[Search] Duration < 3 min, testing if short: ${shortsUrl}`,
          );
          ...
          if (currentUrl.includes("/shorts/")) {
            isShort = true;
            finalUrl = currentUrl;
            console.log(`[Search] Confirmed as short: ${currentUrl}`);
          } else {
            console.log(
              `[Search] Not a short, redirected to: ${currentUrl}. Using original URL.`,
            );
      ...
      console.log(`[Search] Processing ${contentType}: ${finalUrl}`);
      ...
      if (
        ...
        console.log(
          `[Search] Found ${contentType.toLowerCase()} ${results.length}/${maxResult}: ${finalUrl} by @${channelInfo.username}`,
        );
      }
...
function isDurationLessThanThreeMinutes(duration: string): boolean {
